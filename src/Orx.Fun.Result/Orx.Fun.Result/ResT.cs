...
public readonly struct Res<T> : IEquatable<Res<T>>
{
    ...
    /// </code>
    /// </summary>
    /// <param name="map">Function (T -> Res&lt;TOut>) to be called in try-catch block to get the result when Ok; will not be called when Err.</param>
    /// <param name="name">Name of the map function/operation; to be appended to the error messages if the function throws. Omitting the argument will automatically be filled with the function's expression in the caller side.</param>
    public Res<TOut> TryFlatMap<TOut>(Func<T, Res<TOut>> map, [CallerArgumentExpression("map")] string name = "")
    {
        if (Err == null && Val != null)
        {
            try
    ...
    /// <summary>
    /// (async version) <inheritdoc cref="TryFlatMap{TOut}(Func{T, Res{TOut}}, string)"/>
    /// </summary>
    /// <param name="map">Function (T -> Res&lt;TOut>) to be called in try-catch block to get the result when Ok; will not be called when Err.</param>
    /// <param name="name">Name of the map function/operation; to be appended to the error messages if the function throws. Omitting the argument will automatically be filled with the function's expression in the caller side.</param>
    public async Task<Res<TOut>> TryFlatMapAsync<TOut>(Func<T, Task<Res<TOut>>> map, [CallerArgumentExpression("map")] string name = "")
    {
        if (Err == null && Val != null)
        {
            try
